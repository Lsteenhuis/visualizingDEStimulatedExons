...
# analysis
print("starting estimate SizeFactor")
dxd = estimateSizeFactors( dxd )
print("starting estimateDispersions")
dxd = estimateDispersions( dxd,  fitType="parametric")
print("starting plotting of dispersions estimates")
#test <- plotDispEsts( dxd )
pdf("/groups/umcg-wijmenga/tmp04/umcg-lsteenhuis/DEXSEQ/plots/plot_test/dispEst_RPMI_STREP_oneCore.pdf")
plotDispEsts( dxd )
dev.off()

print("starting test for DEU")
#test if the fraction of gene reads that fall onto the exon differens significantly
dxd = testForDEU( dxd
...
#select how many genes are significant with a false discovery rate of 10%
table ( dxr1$padj < 0.1 ).

table( tapply (dxr1$padj < 0.1, dxr1$groupIDS, any  ) )
print("dexseq HTML")
